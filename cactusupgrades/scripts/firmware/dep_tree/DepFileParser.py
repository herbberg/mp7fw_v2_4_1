...
class Command(object):
  #--------------------------------------------------------------
  def __init__( self , aFilePath, aLib, aMap, aInclude , aTopLevel , aComponentPath, aVhdl2008, aPost ):
...
class DepFileParser(object):
  ...
  def __init__( self , aCommandLineArgs , aPathmaker ):
    ...
    subp.add_argument("-d","--descend", action = "count")
    subp.add_argument("-s","--subdir")
    subp.add_argument("-p","--post" , action = "store_true")
    subp.add_argument("--cd")
    subp.add_argument("file", nargs = "*")
  ...
  def parse(self, aFileName, aComponentPath):
    ...
    with open(aFileName) as lDepFile:
      for lLine in lDepFile:
        ...
        if lParsedLine.cmd == 'src' or lParsedLine.cmd == 'include':
        ...
        #--------------------------------------------------------------
        # Specifies the script should be run after all the imports
        if lParsedLine.cmd == 'setup':
          lPost = lParsedLine.post
        else:
          lPost = False
        #--------------------------------------------------------------
        ...
        for lFileExpr in lFileExprList:
          ...
          for lFile in lFileList:    
            ...
            if lParsedLine.cmd == "include":
              ...
              self.CommandList[ lType ].append( Command( lFile, lLib, lMap, lInclude , lTopLevel , lComponentPath, lVhdl2008, lPost ) )
            #--------------------------------------------------------------
